...
const NotificationsModal = () => {
  ...
  const onClose = () => {
    // Navigate back to the previous page when closing the modal
    navigate(-1);
  };

  const filteredNotifications =
    activeTab === 'all' ? notifications : notifications.filter((notification) => notification.category === activeTab);

  return (
    <div className="font-medium fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        {/* Modal panel */}
        <div className="inline-block align-bottom bg-white sm:rounded-tl-[30px] text-left overflow-hidden shadow-xl transform transition-all fixed top-0 right-0 left-0 sm:left-auto sm:right-0 w-full sm:max-w-[504px]">
            ...
            <div className="px-7 pt-9 pb-6 border-b border-gray-200">
              ...
              <div className="flex items-stretch gap-2 mt-5.5 w-full">
                {[
                  ...
                  <div key={tab.id} className={activeTab !== tab.id ? 'flex-1' : 'flex-shrink-0'}>
                    <button
                      onClick={() => setActiveTab(tab.id)}
                      className={`cursor-pointer w-full px-4 py-1.5 text-sm rounded-full whitespace-nowrap border transition-colors ${
                        activeTab === tab.id
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hidden">
              {filteredNotifications.length > 0 ? (
                    ...
                    <div className="flex items-start gap-4 p-6">
                      ...
                      <div className="ml-3 flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-xs text-neu-norm-3 whitespace-nowrap ml-2">{notification.time}</span>
                        </div>

                        <div className="mt-3.5 flex justify-between items-center">
                          <Button variant="secondary" size="small" className="!px-3 !py-1.5 !text-sm ">
                ...
                <div className="p-6 text-center">
                  ...
                  <p className="text-xs text-neu-norm-3 mt-2">We'll let you know when something arrives</p>
                </div>
              )}
            </div>
          </div>
