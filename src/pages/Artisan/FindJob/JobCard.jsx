...
const JobCard = ({ job, activeTab }) => {
  ...
  const handleApplyClick = (e) => {
    ...
    // console.log('🔗 JobCard: Active tab:', activeTab);
    console.log('🔗 JobCard: Navigating to:', `/artisan/find-jobs/${activeTab}/${job.id}`);
  ...
  return (
    <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200 hover:border-blue-200">
      {/* Clickable area for navigation - removed Link wrapper */}
      <div className="block">
        {/* Main content */}
        <div className="">
          ...
          <p
            className="text-gray-700 text-sm leading-relaxed line-clamp-2 h-[50px] cursor-pointer"
            title={description}
            onClick={handleApplyClick}
          >
            {description}
        ...
        {/* Profile section */}
        <div
          className="px-4 py-4 border-y-2 border-gray-200 flex items-center gap-3 my-5.25 cursor-pointer hover:bg-gray-50 transition-colors"
          onClick={handleApplyClick}
        >
        ...
        <div
          className="text-sm text-gray-700 space-y-4 cursor-pointer hover:bg-gray-50 p-2 -m-2 rounded transition-colors"
          onClick={handleApplyClick}
        >
      ...
      <div className="mt-6.5">
        <div className="flex gap-2">
          {isRequest ? (
            ...
            <div className="">
              <button
                className="px-4.25 py-2 bg-blue text-white rounded-2xl hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                onClick={handleApplyClick}
                type="button"
              >
                Apply
