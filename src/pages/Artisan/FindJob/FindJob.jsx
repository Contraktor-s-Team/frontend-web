...
const FindJob = ({
  ...
  // New state to track jobs the user has applied to
  ...
  const tabs = [
    ...
    { id: 'requests', label: 'Direct Jobs Requests' },
  ];
  ...
  // Extract applied job IDs from proposal data
  useEffect(() => {
    if (proposalData && proposalData.length > 0) {
      const appliedJobIds = proposalData.map(proposal => proposal.jobListingId);
  ...
  // Function to handle successful job application
  const handleJobApplicationSuccess = (jobId) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentPageData.map((job) => (
                <JobCard
                  ...
                  onApplicationSuccess={handleJobApplicationSuccess}
                />
              ))}
            </div>
