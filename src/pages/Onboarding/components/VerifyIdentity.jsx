...
const VerifyIdentify = ({ formData, user, onFormChange, onNext, userData, isLoading, error }) => {
  ...
  return (
    <div className="">
      ...
      <div className="flex space-x-4">
        <button
          className="flex-1 py-3 px-6 border border-gray-300 rounded-full text-gray-700 font-medium hover:bg-gray-50 transition-colors"
          onClick={handleSkip}
        >
          Skip
        </button>
        <button
          className="flex-2 py-3 px-6 bg-blue-500 text-white rounded-full font-medium hover:bg-blue-600 transition-colors"
      ...
      <SuccessModal
        ...
        onPrimaryButtonClick={() =>
          Navigate(`${formData?.role === 'user' || user?.data?.role === 'User' ? '/' : '/'}`, {
